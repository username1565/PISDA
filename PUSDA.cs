...
namespace PUPSA
{
	class PUPSA
	{
		...
		// To change this, use:
		//	get:	string file = PUPSA.DBFileName;
		//	set:	PUPSA.DBFileName = "filename.db3";
		...
		// To change this, use:
		//	get:	string str = PUPSA.ConnectionString;
		//	set:	PUPSA.ConnectionString = "value";
		...
		//Or false, if need to keep connection: PUPSA.CloseConnections = false;
		...
		//	get: _cmd = PUPSA.cmd; //cmd will be created, if not exists
...
//COMMIT TRANSACTION; 
		...
		public static void Tests(string dbfile){
			...
			PUPSA.DBFileName = dbfile;	//set filename of DB, to let program regenerate the connection string.
...
;
			...
			//================================================================================================================
			Log("Extract DataSet");
			DataSet dataset = PUPSA.GetDataSet("SELECT * FROM tbl_userinfo");
			LogDataSet(dataset);
			Log("");
			System.Threading.Thread.Sleep(timeout);
			//================================================================================================================
			...
			DataTable datatable = PUPSA.GetDataTable("SELECT * FROM tbl_userinfo");
			...
			Log("Insert records");
			int
			i = PUPSA.ExecuteSQL("INSERT INTO tbl_userinfo (name, age, sex, contact, status) VALUES ('lol', 5, 'Male', 00000, 1)");
			Log(i.ToString());
			i = PUPSA.ExecuteSQL("INSERT INTO tbl_userinfo (name, age, sex, contact, status) VALUES ('lola', 5, 'Female', 00000, 1)");
			Log(i.ToString());
			i = PUPSA.ExecuteSQL("INSERT INTO tbl_userinfo (name, age, sex, contact, status) VALUES ('lol', 5, 'Male', 00000, 1)");
			Log(i.ToString());
			Log("");
...
"
			...
			datatable = PUPSA.GetDataTable("SELECT * FROM AllTypes WHERE id=2", true);
			//Log(datatable);
			Log("");
			System.Threading.Thread.Sleep(timeout);	
			...
			datatable = PUPSA.ExecuteWithResults(sql);
			LogDataTable(datatable);
			Log("");
			System.Threading.Thread.Sleep(timeout);	
		...
		public static void ExecuteSQL(string dbfile, string sql)
		{
			...
			if(sql == ""){
				while(true)
				{
					...
					DataTable datatable = PUPSA.ExecuteWithResults(sql);
					LogDataTable(datatable);
					Log("");
				}
			}else{
		...
		)
		{
			...
			if(args.Length >= 1)						//if args[] exists
			{
				...
				string Usage =
@"
1. PUPSA.exe [""dbfilename""] [""SQLRequest""] - execute SQLRequest specified as string, and exit.
2. PUPSA.exe [""dbfilename""] + input SQLRequests in console.
3. PUPSA.exe (without arguments) - to run tests with ""test.db""
